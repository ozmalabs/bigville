...
`stardew_like_art_direction.png` is the current visual reference for the
palette, silhouette density, tall facades, irregular paths, and held-item
readability. The earlier `pixel_art_art_direction.png` and
`terrain_art_direction.png` references remain useful asset studies. The
runtime assets remain the deterministic atlases generated by
