...
`pixel_art_art_direction.png` is the current visual reference for the palette,
silhouette density, roof/interior contrast, and held-item readability.
`terrain_art_direction.png` is the companion reference for organic meadows,
path corners, shorelines, and the density of small life props. The runtime
assets remain the deterministic atlases generated by
`bigville/game/assets/generate_assets.py`.
